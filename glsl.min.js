...
this.canvas=a.canvas;this.fragment=a.fragment;this.variables=a.variables;this.update=a.update||function(){};this.parseStructs();this.parseUniforms();if(!this.uniformTypes.resolution)throw Error("Glsl: You must use a 'vec2 resolution' in your shader.");delete this.uniformTypes.resolution;for(var c in this.uniformTypes)c in this.variables||l("variable '"+c+"' not initialized");this.initGL();this.load();this.syncAll();a.init&&a.init();this.update(0,0);this.render()};Glsl.supported=function(){return!!r(document.createElement("canvas"))};
